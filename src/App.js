...
function App() {
  ...
  return (
    <div className="">
      ...
      <input className="w-full h-full border py-2 px-4" onChange={event => setTitle(event.target.value)} />
      </div>
          ...
          if (item.descrIns.toLowerCase().includes(title))
            return (
              <div className=" w-1/4" key={item.codW4}>
                <div className="border w-full h-full p-6">{item.descrIns}
                <div className="flex flex-wrap ">
                {item.appelli.map((appello) => (
                    <p className="bg-purple-200 m-1 px-1 rounded-lg">{appello.dataStr}</p>
                ))}
                </div>
                </div>
              </div>
